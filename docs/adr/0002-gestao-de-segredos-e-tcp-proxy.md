...
1. Expõe um endpoint **interno** (`postgres.railway.internal`) acessível apenas de dentro do Railway. O serviço de autenticação (Railway) e a app no K8s local precisam de acesso **externo** ao banco.
...
2. **Segredos:** a **senha nunca é versionada nem exposta** nos outputs. Os outputs entregam *templates* de connection string com o placeholder `${PGPASSWORD}`; a senha é injetada em **runtime** via variável de ambiente / secret (`PGPASSWORD`) — no K8s como Secret, no serviço de auth (Railway) como variável, no GitHub Actions como Secret.
